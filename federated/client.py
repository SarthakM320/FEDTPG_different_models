...
class Client(TrainerBase):
    ...
    def build_model(self,clip_model):
        ...
        if cfg.MODEL.NAME == 'fedtpg':
            ...
            self.model = Fedotp(cfg, clip_model,device = self.device)
    ...
    def train(self,num_round):
        ...
        if hasattr(self.model, 'image_adapter'):
            local_updates = self.model.image_adapter.state_dict()
    ...
    def load_meta(self, global_net):
        ...
        if hasattr(self.model, 'image_adapter'):
            self.model.image_adapter.load_state_dict(global_net)
