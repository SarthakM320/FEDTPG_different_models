...
from model.fedotp import Fedotp, load_clip_to_cpu as load_clip_otp
from model.normal_clip import CLIP, Siglip
from model.fedotp_siglip import FedotpSiglip
...
class Server(TrainerBase):
    ...
    def build_model(self):
        ...
        # clip_model = SiglipModel.from_pretrained("nielsr/siglip-base-patch16-224")
        ...
        elif cfg.MODEL.NAME == 'fedclip':
            self.model = FedClip(cfg, clip_model,device = self.device)
        elif cfg.MODEL.NAME == 'fedotp':
            self.model = Fedotp(cfg,clip_model, self.device)
        elif cfg.MODEL.NAME == 'clip':
            self.model = CLIP(cfg,clip_model, self.device)
        elif cfg.MODEL.NAME == 'siglip':
            self.model = Siglip(cfg,clip_model, self.device)
        elif cfg.MODEL.NAME == 'fedotp_siglip':
            self.model = FedotpSiglip(cfg,clip_model, self.device)
        ...
        if hasattr(self.model, 'prompt_learner'):
            self.register_model("prompt_learner", self.model.prompt_learner)
        if hasattr(self.model, 'image_adapter'):
    ...
    def train(self):
        ...
        if hasattr(self.model, 'image_adapter'):
            self.meta_net_glob = copy.deepcopy(self.model.image_adapter)
        ...
        if hasattr(self.model, 'image_adapter'):
            self.model.image_adapter.load_state_dict(self.meta_net_glob.state_dict())      
